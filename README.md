...
- RAG 智能问答：混合检索（BM25 + 向量）+ 多路召回 + 重排序 + LLM 生成，流式输出（NDJSON），返回答案与引用来源
...
- **检索质量**：已完成混合检索（BM25 + 向量）、重排序（reranker）、查询改写与多路召回，见 [server/README.md](server/README.md#检索链路)。
...
- **性能**：已完成单例复用、检索器缓存、问答流式输出、两级答案缓存、索引分片与增量更新、向量化异步化、前端图表按需引入，以及检索阶段的词法索引惰性构建与版本号失效。
